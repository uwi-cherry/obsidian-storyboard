...
export default function RightSidebarReactView({ view, app }: RightSidebarReactViewProps) {
  ...
  const setCurrentLayerIndex = useCurrentLayerIndexStore(state => state.setCurrentLayerIndex);
...
useEffect(() => {
  ...
  const unsubscribeCurrentFile = globalVariableManager.subscribe(
    GLOBAL_VARIABLE_KEYS.CURRENT_FILE,
    (file: TFile | null) => {
      setCurrentFile(file);
    }
  );
