  ...
  useEffect(() => {
    ...
    try {
      ...
      project.timeline.tracks.forEach(track => {
        track.children.forEach((clip: OtioClip) => {
          if (clip.media_reference.target_url && clip.media_reference.target_url.trim()) {
            try {
              // ビデオファイルの場合
              if (clip.media_reference.target_url.match(/\.(mp4|webm|ogg)$/i)) {
              ...
              // 画像ファイルの場合
              else if (clip.media_reference.target_url.match(/\.(jpg|jpeg|png|gif|webp)$/i)) {
                const imageLayer = new etro.layer.Image({
                  startTime: clip.source_range.start_time,
                  duration: clip.source_range.duration,
                  source: clip.media_reference.target_url,
                  x: 0,
                  y: 0,
                  width: 640,
                  height: 360
