...
const StoryboardReactView: React.FC<StoryboardReactViewProps> = ({
  ...
  const handleDeleteChapter = (chapterIndex: number) => {
    ...
    setOpenChapters(prev => prev.filter((_, idx) => idx !== chapterIndex));
  };
      ...
      <table className="w-full border-collapse border border-modifier-border mb-32 table-fixed">
        <thead>
          <tr className="bg-secondary">
            {columns.map(col => (
              <th key={String(col.key)} className="border border-modifier-border px-4 py-2 text-left">
                {col.header}
              </th>
            ))}
        ...
        <tbody>
          {storyboard.chapters.map((chapter, cIdx) => (
            <tr key={cIdx} className="align-top">
              <td colSpan={columns.length} className="border border-modifier-border p-0">
                <details
                  open={openChapters[cIdx]}
                  onToggle={e => {
                    const open = (e.currentTarget as HTMLDetailsElement).open;
                    setOpenChapters(prev => prev.map((o, idx) => (idx === cIdx ? open : o)));
                  }}
                >
                  <summary className="font-bold px-2 py-2">
                    <input
